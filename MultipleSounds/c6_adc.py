"""
4-bit successive-approximation ADC reader (C6 ADC).

Implements a basic SAR ADC using:
  - PWM output as a DAC (through an RC filter)
  - Comparator digital output as the decision bit

Primary entry point:
    read_adc2() -> float  (estimated voltage, assuming 3.3V reference)
...
DAC_SETTLE_S = 0.05  # was 0.10 in earlier versions
...
bit3_2, bit2_2, bit1_2, bit0_2 = 1, 0, 0, 0
...
def setup_gpio_and_pwm() -> "GPIO.PWM":
    ...
    pwm = GPIO.PWM(PWM_PIN2, PWM_FREQ_HZ)
    return pwm
...
def adc_step(pwm: "GPIO.PWM", clk: int) -> None:
    ...
    Preserves the original step ordering and logic:
      clk == 2 : decide bit2 (and possibly bit3)
      clk == 4 : decide bit1 (and possibly bit2)
      clk == 6 : decide bit0 (and possibly bit1)
      clk == 8 : finalize bit0
    """
    global bit3_2, bit2_2, bit1_2, bit0_2

    v3, v2, v1, v0 = bit3_2, bit2_2, bit1_2, bit0_2
    count = 0

    if clk == 2:
        time.sleep(FIRST_CYCLE_EXTRA_SETTLE_S)

    c1 = GPIO.input(COMPARATOR2)

    if clk == 2:
        if c1 > 0:
            v2 = 1
            count = 1
        elif c1 == 0 and bit2_2 == 0 and count == 0:
            v3 = 0
            v2 = 1
        ...
        if c1 > 0:
            v1 = 1
            count = 1
        elif c1 == 0 and bit1_2 == 0 and count == 0:
            v2 = 0
            v1 = 1
        ...
        if c1 > 0:
            v0 = 1
            count = 1
        elif c1 == 0 and bit0_2 == 0 and count == 0:
            v1 = 0
            v0 = 1
    ...
    bit3_2, bit2_2, bit1_2, bit0_2 = v3, v2, v1, v0
    dac_from_bits(pwm, bit3_2, bit2_2, bit1_2, bit0_2)
...
def read_adc2() -> float:
    ...
    try:
        ...
        digital_value = (bit3_2 * 8) + (bit2_2 * 4) + (bit1_2 * 2) + bit0_2
...
#     try:
#         print("Measured voltage:", read_adc2())
